...
int step2()
{
  ...
  int isOwner = 1;
  SQLite_setProperty(&testMyObject->object, PROPERTY_isOwner, &isOwner);
  isOwner = 0;
  SQLite_getProperty(&testMyObject->object, PROPERTY_isOwner, &isOwner);
  PRINT(("IsOwner = %d\n", isOwner));
  //MyObject_delete(testMyObject);
