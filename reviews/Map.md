...
The Map class is a high-level collection built on top of your Object framework. It typically manages a collection of MapEntry objects.
...
* Ownership: In your MapEntry_new function, ensure you are calling Object_getRef() on both the key and the value. This ensures they aren't deleted while they are inside the map.
...
## 4. Potential Memory Leak in Map_insert

If your Map_insert logic overwrites an existing key but doesn't deRef the old MapEntry, you will have a memory leak.

## 5. Excessive Hash calculation and collisions

The has is calculated several time for all substring up to the length of the string.

## 6. Get size incorrect implementation

get_size(object) should return the size on bytes of the object while the class size should be statically associated to the class.

## 7. Clarify the ownership semantic

TBD

## 8. Allow to resize the Map object
